...
from lamellar import full_mcg, guessd
from itfit import itfit
import outfit

filepatt = "/Users/jwinnikoff/Documents/MBARI/SAXS/HPSAXS_7A_May2021/20210507/DOPE/DOPE_*_20C_*00000[1-9].dat"
paramtab = '/'.join(filepatt.split('/')[:-1]) + filepatt.split('/')[-1].split('_')[0] + "_params.tsv"
...
def fit_one_lamellar(data):
    ...
    fitsteps = (
        {"fixpar":{'N':25, 'nu':0.07, 'zH':20, 'sigH':3, 'sigC':4, 'rhor':-1, 'Nu':0, 'offs':0}, "freepar":{'d':guessd(data[0], data[1]), 'scal':0.0001}},
...
with open(paramtab, 'w') as handout:
    outfit.params_df(fitlist).to_csv(handout, sep='\t', index=False)
